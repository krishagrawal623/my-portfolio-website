...
export const Model = ({
  ...
  useEffect(() => {
    if (!loaded || !modelGroup.current) return;
    modelGroup.current.traverse(node => {
      if (node.material && node.name !== MeshType.Screen && node.name !== MeshType.Logo) {
        node.material.color = new Color(theme === 'light' ? 0xcccccc : 0x1f2025);
      }
    });
    renderFrame();
  }, [theme, loaded, renderFrame]);
